...
void character_tests() {
        ...
        if (char1.getName() != "john" || char2.getName() != "john") {
                std::cout << "Failed to copy name in copy assignment!"
                          << std::endl;
        }

        char1 = *static_cast<Character *>(me);
        std::cout << "steve should be able to cast both ice and cure on bob"
                  << std::endl;
        char1.use(0, *bob);
        char1.use(1, *bob);
        char1.use(3, *bob);
...
void materia_tests() {
        ...
        if (dynamic_cast<Ice *>(copy) == NULL) {
                std::cout << "If you see this message, a test failed"
                          << std::endl;
        }
