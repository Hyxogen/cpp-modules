...
typename ft::enable_if<(not ft::is_same<To, char>::value), void>::type
display_conversion(const From &from) {
...
template<typename To, typename From>
typename ft::enable_if<ft::is_same<To, char>::value, void>::type
display_conversion(const From &from) {
        const std::string name = "char";
        if (from > static_cast<From>(std::numeric_limits<char>::max())
            || from < static_cast<From>(std::numeric_limits<char>::min())) {
                std::cout << name << ": impossible" << std::endl;
        } else if (!isprint(static_cast<int>(from))) {
                std::cout << name << ": Non displayable" << std::endl;
        } else {
                std::cout << name << ": " << static_cast<char>(from)
                          << std::endl;
        }
}
...
int main(int argc, char **argv) {
        if (argc != 2) {
                std::cerr << "usage " << argv[0] << " literal|pseudo_literal"
                          << std::endl;
                return 0;
        }
        const int i = 500;
