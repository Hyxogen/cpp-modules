...
ShrubberyCreationForm::ShrubberyCreationForm() :
    Form(SHRUB_NAME, SHRUB_SIGN_GRADE, SHRUB_EXEC_GRADE), _target("unknown") { }
ShrubberyCreationForm::ShrubberyCreationForm(const std::string &target) :
    Form(SHRUB_NAME, SHRUB_SIGN_GRADE, SHRUB_EXEC_GRADE), _target(target) { }
...
void ShrubberyCreationForm::execute(const Bureaucrat &bcrat) const {
        ...
        if (!file) {
                throw std::ios_base::failure(
                    "failed to open file in ShrubberyCreationForm execute");
        }
        file << "/" << std::endl
             << "├── bin" << std::endl
             << "├── boot" << std::endl
